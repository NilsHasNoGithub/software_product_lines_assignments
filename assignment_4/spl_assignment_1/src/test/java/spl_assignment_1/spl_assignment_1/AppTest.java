...
public class AppTest {
    ...
    public void testServerClientCommunication() throws IOException, InterruptedException {
        ...
        Authenticator[] auths = new Authenticator[] { new NoAuth(), new PlainPasswordAuth("abcdefg") };
        ...
        for (Authenticator auth : auths) {
            ...
            final Server server = new Server(port, encDec, auth);
            final Client client1 = new Client(port, string, encDec, "client1", auth);
            final Client client2 = new Client(port, string, encDec, "client2", auth);
